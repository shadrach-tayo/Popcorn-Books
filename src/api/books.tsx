...
export const booksPlaceHolder: IBook[] = Array.from({ length: 5 }).map((v, i) => ({ ...bookPlaceholder, id: `${i}` }));

export function useBook(bookId: string) {
  return useQuery<IBook, any>({ queryKey: [tags.book, { bookId }], queryFn: () => client(`${API_URL}/${bookId}?key=${APIKEY}`), placeholderData: bookPlaceholder, enabled: !!bookId })
  // return queryClient.getQueryData([tags.book, { bookId }]) as IBook
}

function getSearchConfig(query: string) {
  return ({
    queryKey: [tags.bookSearch, { query }],
    queryFn: () => client(`${API_URL}?key=${APIKEY}&q=${query}&maxResults=5`).then(data => data.items),
    enabled: !!query,
    placeholderData: booksPlaceHolder,
    onSuccess(data: IBook[]) {
      data.forEach(book => queryClient.setQueryData([tags.book, { bookId: book.id }], book))
    }
  })
}
